...
export default function OccasionCard({ occasion }) {
  ...
  return (
    <div
      className="group relative flex flex-col justify-between bg-white rounded-2xl border border-[#E8D9C5]/90 hover:border-[#A67C3A] p-3 sm:p-4 text-center transition-all duration-300 hover:-translate-y-1.5 hover:shadow-xl shadow-xs cursor-pointer h-full select-none"
    ...
    >
          ...
          <div className="w-full aspect-square rounded overflow-hidden relative bg-stone-100 flex items-center justify-center">
            {occasion.image && !imgError ? (
              ...
              <div className="w-full h-full flex flex-col items-center justify-center bg-gradient-to-br from-amber-50 to-stone-100 p-3 text-center">
                <Sparkles size={22} className="text-amber-700 mb-1" />
      ...
      <div className="flex flex-col justify-between flex-1 mt-1 px-1">
        <div>
          <h3 className="text-sm sm:text-base font-bold text-stone-900 group-hover:text-amber-800 transition-colors font-serif leading-snug line-clamp-1">
            {occasion.title}
          </h3>
        ...
        <div className="mt-2.5 inline-flex items-center justify-center gap-1 text-xs font-bold text-amber-800 group-hover:text-amber-900 transition-colors">
